...
methods = {"quantlin": QuantLinear(), "nf4": None}
...
class GPTQQuantizedWeight(nn.Module):
    ...
    def get_qweight(self) -> torch.IntTensor:
        ...
        assert (self.qweight is None) != (
            self.qweight_storage is None
        ), "must have either .codes or storage, but not both"
    ...
    def get_qzeros(self) -> torch.IntTensor:
        ...
        assert (self.qzeros is None) != (
            self.qzeros_storage is None
        ), "must have either .codes or storage, but not both"
